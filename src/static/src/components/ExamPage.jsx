...
const ExamPage = () => {
  ...
  const finishExam = async () => {
    ...
    try {
      ...
      await api.saveExamResult(resultData);
      console.log('Résultat sauvegardé avec succès sur le serveur');
    } catch (error) {
      ...
      console.log('Résultat sauvegardé localement comme fallback');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-white to-yellow-50 flex items-center justify-center">
        <div className="result-card max-w-2xl w-full mx-4">
          <div className="text-center mb-8">
            ...
            {saving && (
              <p className="text-sm text-blue-600 mt-2">Sauvegarde en cours...</p>
            )}
          </div>
